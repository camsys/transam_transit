...
#
#------------------------------------------------------------------------------
class TransitEarlyDispositionRequestUpdatesTemplateBuilder < TemplateBuilder
  ...
  def add_rows(sheet)
    @asset_types.each do |asset_type|
      assets = @organization.assets.operational.order(:asset_type_id, :asset_subtype_id, :asset_tag)
      assets.each do |asset|
        next if asset.disposable?(true)
  ...
  def setup_workbook(workbook)
    ...
    row = []
    @disposition_types = DispositionType.active.where.not(name: ['Transferred','Unknown']).pluck(:name)
  ...
  def post_process(sheet)
    ...
    sheet.add_data_validation("G3:G1000", {
        ...
        :promptTitle => 'Explanation',
        :prompt => 'Explain request'})

    # Disposition Type
    sheet.add_data_validation("H3:H1000", {
        :type => :list,
        :formula1 => "lists!$A$1:$#{alphabet[@disposition_types.size-1]}$1",
        :allow_blank => true,
        :showErrorMessage => true,
        :errorTitle => 'Wrong input',
  ...
  def column_styles
    ...
    if include_mileage_columns?
      styles.concat([{:name => 'disposition_report_integer', :column => 10}])
    end
    styles
  end

  def column_widths
    widths = [nil] * 6 + [40] + [20] * 4
    if include_mileage_columns?
      widths.concat([20])
    end
  ...
  def styles
    ...
    a << {:name => 'explanation_col', :bg_color => "F2DCDB", :fg_color => '000000', :b => false, :alignment => { :horizontal => :center }, :locked => false}
