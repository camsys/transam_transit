      ...
      first_row.upto(reader.last_row) do |row|
        ...
        if reader.empty_row?
          ...
          unless asset.asset_subtype.name == subtype_str
            add_processing_message(2, 'warning', "Mismatch on asset subtype. Found tag '#{subtype_str}' expected '#{asset.asset_subtype.name}'. Skipping row.")
            @num_rows_failed += 1
            next
          end
          ...
          # Disposition Update Events
          add_processing_message(2, 'success', 'Processing Disposition Report')
          if reader.empty?(7, 17)
            @num_rows_skipped += 1
            add_processing_message(2, 'warning', "No data for row. Skipping.")
          else
