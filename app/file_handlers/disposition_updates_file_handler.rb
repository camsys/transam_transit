    ...
    begin
      ...
      first_row.upto(reader.last_row) do |row|
        ...
        if reader.empty_row?
          ...
          if is_number?(asset_tag)
            asset_tag = asset_tag.to_s
          end
          ...
          if reader.empty?(4, 12)
            @num_rows_skipped += 1
            add_processing_message(2, 'info', "No data for row. Skipping.")
          else
            add_processing_message(2, 'success', 'Processing Disposition Update Section')
            loader = DispositionUpdateEventLoader.new            
            loader.process(asset, cells[4..12])
            if loader.errors?
              row_errored = true
