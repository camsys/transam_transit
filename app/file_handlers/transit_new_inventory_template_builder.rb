class TransitNewInventoryTemplateBuilder < UpdatedTemplateBuilder
  ...
  SERVICE_STATUS_COL = 76
  CLASS_COL = 4
  ESL_COL = 7
  FUNDING_TYPE_COL = 40
  CAPITAL_RESP_COL = 41
  PCNT_CAPITAL_RESP_COL = 42
  OWNERSHIP_TYPE_COL = 43
  OWNERSHIP_OTHER_COL = 44
  PURCHASED_NEW_COL = 45
  MODEL_COL = 10
  MODEL_OTHER_COL = 11
  FUEL_TYPE_COL = 15
  ...
  PRIMARY_MODE_COL = 57
  SERVICE_TYPE_PRIMARY_MODE_COL = 58
  DEDICATED_ASSET_COL = 61
  ...
  def setup_lookup_sheet(workbook)
    ...
    row = InfrastructureSegmentType.where(fta_asset_class_id: FtaAssetClass.find_by(name: 'Guideway')).order('name ASC').active.pluck(:name)
    @lookups['guideway_segment_type'] = {:row => row_index, :count => row.count}
    sheet.add_row row
    row_index+=1

    row = InfrastructureSegmentType.where(fta_asset_class_id: FtaAssetClass.find_by(name: 'Power & Signal')).order('name ASC').active.pluck(:name)
    @lookups['power_signal_segment_type'] = {:row => row_index, :count => row.count}
    sheet.add_row row
    row_index+=1
