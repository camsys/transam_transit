class InventoryUpdatesFileHandler < AbstractFileHandler
  include Rails.application.routes.url_helpers # let us link directly to assets, events
  ...
  def process(upload)
    ...
    add_processing_message(1, 'success', "Updating assets from '#{upload.original_filename}'")
    ...
    begin
      ...
      first_row.upto(reader.last_row) do |row|
        ...
        if reader.empty_row?
          ...
          asset = Asset.find_by(organization_id: organization.id, object_key: cells[OBJECT_KEY_COL].to_s)

          # complain if we cant find it
          if asset.nil?
            add_processing_message(2, 'warning', "Could not retrieve asset with asset tag '#{asset_tag}'.")
            @num_rows_failed += 1
            next
          else
            add_processing_message(1, 'success', "Processing row[#{row}]  Asset ID: '#{asset_tag}', Subtype: '#{subtype_str}', Asset Tag: '#{asset_tag}'")
          end
          ...
          if reader.empty?(7, 8) and reader.empty?(10, 11) and reader.empty?(13, 14)
            @num_rows_skipped += 1
          ...
          #### ServiceStatus Report Block #########################################
          add_processing_message(2, 'success', 'Processing Service Status Report')
          if reader.empty?(7, 7)
            add_processing_message(3, 'warning', "No Service Status Information")
          else
            loader = ServiceStatusUpdateEventLoader.new
            loader.process(asset, cells[7..8])
            if loader.errors?
              row_errored = true
          ...
          if reader.empty?(10, 10)
            add_processing_message(3, 'warning', 'No Condition Information')
          else
            loader = ConditionUpdateEventLoader.new
            loader.process(asset, cells[10..11])
            if loader.errors?
              row_errored = true
              loader.errors.each { |e| add_processing_message(3, 'warning', e)}
            end
            ...
            if event.valid?
              event.upload = upload
              event.save
              add_processing_message(3, 'success', 'Condition updated')
              has_new_event = true
            else
          ...
          if asset.type_of? :vehicle or asset.type_of? :support_vehicle
            ...
            if reader.empty?(13, 13) # Only Current Mileage field is required
              add_processing_message(3, 'warning', 'No Mileage Information')
            else
              loader = MileageUpdateEventLoader.new
              loader.process(asset, cells[13..14])
              if loader.errors?
                row_errored = true
                loader.errors.each { |e| add_processing_message(3, 'warning', e)}
              end
              ...
              if event.valid?
                event.upload = upload
                event.save
                add_processing_message(3, 'success', 'Mileage updated.')
                has_new_event = true
              else
