class InfrastructureTamPolicyServiceLifeReport < AbstractReport
  ...
  def self.get_underlying_data(organization_id_list, params)
    ...
    query = Track.operational.joins(:infrastructure_segment_type, :infrastructure_division, :infrastructure_subdivision, :infrastructure_track_type)
    ...
    cols = ['organizations.short_name', 'fta_asset_categories.name', 'fta_asset_classes.name', 'fta_track_types.name', 'asset_subtypes.name', 'infrastructures.from_line', 'infrastructures.from_segment', 'infrastructures.to_line', 'infrastructures.to_segment', 'infrastructures.segment_unit', 'infrastructures.from_location_name', 'infrastructures.to_location_name', 'transam_assets.description', 'transam_assets.asset_tag', 'transam_assets.external_id',  'infrastructure_segment_types.name', 'infrastructure_divisions.name', 'infrastructure_subdivisions.name', 'infrastructure_track_types.name', 'transam_assets.in_service_date', 'transam_assets.purchase_date', 'transam_assets.purchase_cost', 'IF(transam_assets.purchased_new, "YES", "NO")', 'IF(IFNULL(sum_extended_eul, 0)>0, "YES", "NO")', 'IF(transit_assets.pcnt_capital_responsibility > 0, "YES", "NO")', 'infrastructures.max_permissible_speed', 'infrastructures.max_permissible_speed_unit',
            'YEAR(CURDATE()) - YEAR(in_service_date)','rating_event.assessed_rating']

    labels = ['Agency','Asset Category', 'Asset Class', 'Asset Type','Asset Subtype', 'Line','From', 'Line',	'To',	'Unit',	'From (location name)',	'To (location name)',	'Description / Segment Name',	'Asset / Segment ID',	'External ID',	'Segment Type',	'Main Line / Division',	'Branch / Subdivision',	'Track',	'In Service Date',	'Purchase Date',	'Purchase Cost',	'Purchased New',	'Rehabbed Asset?',	'Direct Capital Responsibility', 	'Maximum Permissible Speed',	'Unit',	'Speed Restriction',	'Unit',	'Period',	'Restriction Cause',	'Line',	'From',	'Line',	'To',	'Unit',	'Date & Time of Performance Restriction',	'Track Miles  (Performance Restriction)',	'Age',	'Current Condition (TERM)']
  ...
  def self.get_detail_data(organization_id_list, params)
    ...
    unless key.blank?
      ...
      grouped_performance_restrictions = PerformanceRestrictionUpdateEvent.where(transam_asset: query).group('organizations.short_name').group('CONCAT(fta_mode_types.code," - " ,fta_mode_types.name)')

      assets_count = grouped_query.distinct.count('transam_assets.id')
      line_lengths = grouped_query.distinct.sum('to_segment - from_segment')
      restriction_lengths = grouped_performance_restrictions.distinct.sum('asset_events.to_segment - asset_events.from_segment')

      total_age = grouped_query.sum('YEAR(CURDATE()) - YEAR(in_service_date)')

      line_lengths.each do |k, v|
        assets = query.where(fta_mode_types: {name: asset_level_class.classify.constantize.find_by(name: k.split('-').last.strip)}, organization_id: organization_id_list)
  ...
  def get_data(organization_id_list, params)
    ...
    grouped_performance_restrictions = PerformanceRestrictionUpdateEvent.where(transam_asset: query).group('CONCAT(fta_mode_types.code," - " ,fta_mode_types.name)')
