...
class RevenueVehicleTamServiceLifeReport < AbstractTamServiceLifeReport
  ...
  def get_underlying_data(organization_id_list, params)
    ...
    fta_asset_category = FtaAssetCategory.find_by(name: 'Revenue Vehicles')
    ...
    unless policy.empty?
      cols = ['organizations.short_name', 'fta_asset_categories.name', 'fta_asset_classes.name', vehicle_type, 'asset_subtypes.name', 'transam_assets.asset_tag', 'transam_assets.external_id',  'service_vehicles.serial_number', 'service_vehicles.license_plate', 'transam_assets.manufacture_year', 'CONCAT(manufacturers.code,"-", manufacturers.name)', manufacturer_model, 'CONCAT(fuel_types.code,"-", fuel_types.name)', 'transam_assets.in_service_date', 'transam_assets.purchase_date', 'transam_assets.purchase_cost', 'IF(transam_assets.purchased_new, "YES", "NO")', 'IF(IFNULL(sum_extended_eul, 0)>0, "YES", "NO")', 'IF(transit_assets.pcnt_capital_responsibility > 0, "YES", "NO")',"max_fy_year",'ulbs.useful_life_benchmark + FLOOR(IFNULL(sum_extended_eul, 0)/12)','ulbs.pcnt_goal','IF(ulbs.state = "pending_activation", "Pending Activation", IF(ulbs.state = "activated", "Activated", ""))', "YEAR(CURDATE()) - transam_assets.manufacture_year","transam_assets.manufacture_year + ulbs.useful_life_benchmark + FLOOR(IFNULL(sum_extended_eul, 0)/12)","ulbs.useful_life_benchmark + FLOOR(IFNULL(sum_extended_eul, 0)/12) - (YEAR(CURDATE()) - transam_assets.manufacture_year)",'rating_event.assessed_rating','mileage_event.current_mileage','service_status_types.name']

      labels =[ 'Agency','Asset Category', 'Asset Class', 'Asset Type','Asset Subtype', 'Asset ID',  'External ID',  'VIN','License Plate',  'Manufacturer Year',  'Manufacturer', 'Model',  'Fuel Type',  'In Service Date', 'Purchase Date', 'Cost (Purchase)',  'Purchased New', 'Rehabbed Asset?', 'Direct Capital Responsibility','TAM Policy Year', 'ULB Goal', 'Goal Pcnt','Tam Policy Status','Age', 'Replacement Year (TAM Policy)', 'Useful Life Remaining','Current Condition (TERM)', 'Current Mileage (mi.)', 'Current Service Status']

      formats = [:string, :string, :string, :string, :string, :string, :string, :string, :string, :integer, :string, :string, :string, :date, :date, :currency, :string, :string, :string, :fiscal_year, :integer, :percent, :string, :integer, :integer, :integer, :decimal, :integer, :string]
    else
      ...
      labels =[ 'Agency','Asset Category', 'Asset Class', 'Asset Type','Asset Subtype', 'Asset ID',  'External ID',  'VIN','License Plate',  'Manufacturer Year',  'Manufacturer', 'Model',  'Fuel Type',  'In Service Date', 'Purchase Date', 'Cost (Purchase)',  'Purchased New', 'Rehabbed Asset?', 'Direct Capital Responsibility','Age','Current Condition (TERM)', 'Current Mileage (mi.)', 'Current Service Status']

      formats = [:string, :string, :string, :string, :string, :string, :string, :string, :integer, :string, :string, :string, :date, :date, :currency, :string, :string, :string, :integer, :decimal, :integer, :string]
    end
  ...
  def get_data(organization_id_list, params)
    ...
    asset_counts.each do |k, v|
      ...
      mileage_events = MileageUpdateEvent
                      .where(id: AssetEvent
      ...
      data << (single_org_view ? [] : ['All (Filtered) Organizations']) + [*k, v, (tam_data[k] || [])[0], (tam_data[k] || [])[1], (tam_data[k] || [])[2], tam_data[k] ? past_ulb_counts[k].to_i : '',tam_data[k] ? (past_ulb_counts[k].to_i*100/v.to_f+0.5).to_i: '', (total_age[k].to_i/v.to_f).round(2), condition_events_count > 0 ? condition_events.sum(:assessed_rating)/condition_events_count.to_f : '', mileage_events_count > 0 ? (mileage_events.sum(:current_mileage)/mileage_events_count.to_f + 0.5).to_i : '' ]
    end
