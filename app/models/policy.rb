...
class Policy < ActiveRecord::Base
  ...
  # Override the default method and allow the policy to search for assets based on an optional fuel type
  def get_rule(asset)
    if asset.type_of? :rolling_stock
      policy_items.where(:asset_subtype => asset.asset_subtype, :fuel_type => asset.fuel_type).first      
    else
