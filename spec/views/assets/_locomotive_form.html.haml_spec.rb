...
describe "assets/_locomotive_form.html.haml", :type => :view do
  it 'fields' do
    ...
    expect(rendered).to have_xpath('//input[@id="asset_primary_fta_service_type_id_1"]')
    expect(rendered).to have_field('asset_secondary_fta_mode_type_id')
    expect(rendered).to have_field('asset_secondary_fta_service_type_id')
    expect(render)
