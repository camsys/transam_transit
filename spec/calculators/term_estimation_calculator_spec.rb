...
RSpec.describe TermEstimationCalculator, :type => :calculator do
  ...
  describe '#calculate' do
    it 'vehicle under 3yo calculates' do
      test_asset = create(:bus, {:in_service_date => Date.today - 2.years, :asset_type => AssetType.first})
    ...
    it 'vehicle over 3yo calculates' do
      test_asset = create(:bus, {:in_service_date => Date.today - 4.years, :asset_type => AssetType.first})
      mileage_update_event = create(:mileage_update_event, :asset => test_asset)
      expect(test_calculator.calculate(test_asset).round(4)).to eq(3.0716)
    end

    it 'rail car under 2.5yo calculates' do
      test_asset = create(:light_rail_car, {:in_service_date => Date.today - 2.years, :asset_type => AssetType.find_by(:class_name => "RailCar")})
      mileage_update_event = create(:mileage_update_event, :asset => test_asset)
    ...
    it 'rail car over 2.5yo calculates' do
      test_asset = create(:light_rail_car, {:in_service_date => Date.today - 4.years, :asset_type => AssetType.find_by(:class_name => "RailCar")})
      mileage_update_event = create(:mileage_update_event, :asset => test_asset)
    ...
    it 'support facility under 18yo calculates' do
      test_asset = create(:administration_building, {:in_service_date => Date.today - 2.years, :asset_type => AssetType.find_by(:class_name => "SupportFacility")})

      expect(test_calculator.calculate(test_asset).round(4)).to eq(4.89)
    end

    it 'support facility over 18yo calculates' do
      test_asset = create(:administration_building, {:in_service_date => Date.today - 20.years})
      expect(test_calculator.calculate(test_asset).round(4)).to eq(3.2033)
    end

    it 'transit facility calculates' do
      test_asset = create(:bus_shelter, {:in_service_date => Date.today - 2.years})
    ...
    it 'fixed guideway calculates' do
      ...
      test_asset = create(:fixed_guideway, {:in_service_date => Date.today - 2.years})
  ...
  describe '#last_serviceable_year' do
    it 'vehicle calculates' do
      test_asset = create(:bus, {:in_service_date => Date.today - 2.years})
    ...
    it 'rail car calculates' do
      test_asset = create(:light_rail_car, {:in_service_date => Date.today - 2.years})
    ...
    it 'support facility calculates' do
      test_asset = create(:administration_building, {:in_service_date => Date.today - 2.years})
