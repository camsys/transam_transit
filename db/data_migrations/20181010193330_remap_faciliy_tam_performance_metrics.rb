    ...
    TamGroup.joins(:fta_asset_categories).where(fta_asset_categories: {name: 'Facilities'}).each do |group|
      FtaAssetClass.all.each do |fta_asset_class|
        ...
        if old_metrics.count > 0
          ...
          if old_metrics.distinct.pluck(:useful_life_benchmark, :pcnt_goal).count == 1
            ...
            new_metric.update(asset_level: fta_asset_class, useful_life_benchmark: 3, pcnt_goal: old_metrics.maximum(:pcnt_goal))
          end
