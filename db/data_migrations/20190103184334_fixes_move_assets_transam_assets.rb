class FixesMoveAssetsTransamAssets < ActiveRecord::DataMigration
  def up
    ...
    # fix bad mapping of old Asset to TransamAsset of fta_type for Fare Equipment
    TransitAsset.where(asset: Asset.where(asset_subtype: AssetSubtype.where(name: 'Fare Collection Systems'))).update_all(fta_type_type: 'FtaEquipmentType', fta_type_id: FtaEquipmentType.find_by(name: 'Fare Equipment').id)
