...
# TransamTransitCoverageBaseline, not at file/Object scope. `namespace do...end`
...
# Object -- the same names transam_core's identical-shaped file also assigns.
# Once transit's dummy app loads transam_core as an engine, the two collide.
...
module TransamTransitCoverageBaseline
...
namespace :transam_transit do
  ...
  task :check_coverage_baseline do
    ...
    manifest = YAML.load_file(TransamTransitCoverageBaseline::MANIFEST_PATH)
    ...
    rspec = TransamTransitCoverageBaseline.load_rspec_coverage(root)
    ...
    puts "== transam_transit coverage baseline (#{TransamTransitCoverageBaseline::MANIFEST_PATH}) =="
    puts "%-38s %8s %8s %8s" % ["group", "covered", "relevant", "percent"]
    group_rows.each { |name, c, r| puts "%-38s %8d %8d %7.2f%%" % [name, c, r, r.zero? ? 0.0 : c * 100.0 / r] }
    puts "-" * 68
    puts "%-38s %8d %8d %7.2f%%  (floor %d, delta %+d; informational relevant %d, delta %+d)" %
  ...
  task :pin_coverage_baseline do
    ...
    header = File.readlines(TransamTransitCoverageBaseline::MANIFEST_PATH).take_while { |l| l.start_with?('#') }.join
    File.write(TransamTransitCoverageBaseline::MANIFEST_PATH, header + manifest.to_yaml)
    puts "Re-pinned #{TransamTransitCoverageBaseline::MANIFEST_PATH}: covered=#{total_covered} relevant=#{total_relevant}"
  end
end
